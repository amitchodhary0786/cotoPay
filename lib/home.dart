      ...
      child: Scaffold(
        ...
        body: IndexedStack(
          ...
          children: [
            ...
            //Container(), // Placeholder for 'Vouchers'
            UpiVouchersScreen(),
...
class HomeContent extends StatelessWidget {
              ...
              child: Column(
                children: [
                    ...
                    child: FutureBuilder<List<dynamic>>(
                      ...
                      builder: (context, snapshot) {
                        ...
                        final createdVouchers = snapshot.hasData
                            ? snapshot.data!.where((voucher) => voucher['type'] == 'Active').toList()
                            : [];
                                  ...
                                  child: Column(
                                      ...
                                      children: [
                                        ...
                                        ElevatedButton.icon(
                                            ...
                                            onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const HowUpiVouchersWorks()))),
                                      ])));
                        }
                  ...
                  Padding(
                      ...
                      child: _buildActionButtons(screenWidth: screenWidth)),
                  const SizedBox(height: 16),
                  _buildOffersSection(),
                ],
              ),
  ...
  Widget _buildVoucherCard(BuildContext context,
    ...
    // Scale fonts based on cardWidth
    ...
    return GestureDetector(
      onTap: () {
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => VoucherDetailScreen(voucherData: voucherData)),
        );
      },
      child: Container(
        width: cardWidth,
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    title,
                    style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: titleFont),
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
                  ),
                ),
  ...
  Widget _buildActionButtons({required double screenWidth}) {
    // Make buttons responsive: use Wrap so they wrap on small screens
  ...
  Widget _buildVoucherBalanceCard({required int count, required double totalAmount}) {
    String displayAmount = NumberFormat.currency(locale: 'en_IN', symbol: '₹', decimalDigits: 2).format(totalAmount);
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
              const Text('ACTIVE VOUCHERS', style: TextStyle(color: Colors.white70, fontSize: 12, letterSpacing: 0.5)),
              const SizedBox(width: 8),
              Container(
                  padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                  decoration: BoxDecoration(color: const Color(0xff48484A), borderRadius: BorderRadius.circular(10)),
                  child: Text(count.toString(), style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.bold))),
              const SizedBox(width: 8),
              const Icon(Icons.info_outline, color: Colors.white54, size: 16)
            ]),
            Text(displayAmount, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 22))
          ])
        ]));
  }
      ...
      child: Column(
        ...
        children: [
          ...
          const Padding(
            ...
            child: Text('OFFERS ON VOUCHERS', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: Colors.black87, letterSpacing: 0.5)),
          ),
          ...
          const SizedBox(height: 90), // Space for bottom nav bar
        ],
      ),
    );
  }
  ...
  Widget _offerTile({required IconData icon, required String title, required String subtitle}) {
    return ListTile(
      leading: Container(
        ...
        child: Icon(icon, color: const Color(0xff34A853), size: 24),
      ),
