...
class _FilterBottomSheet extends StatefulWidget {
  final Function(
          Map<String, bool> selectedCategories, String? selectedTimePeriod)
      onApplyFilter;
...
class _HistoryScreenState extends State<HistoryScreen>
  ...
  final TextEditingController _vouchersSearchController =
  ...
  final TextEditingController _transactionsSearchController =
      TextEditingController();
  ...
  Map<String, bool> _voucherSelectedCategories = {
    'Fuel': false,
    'Meal': false,
    'Travel': false,
    'Accommodation': false,
  ...
  Map<String, bool> _transactionSelectedCategories = {
    'Fuel': false,
    'Meal': false,
    'Travel': false,
    'Accommodation': false,
  ...
  void initState() {
    ...
    _tabController.addListener(() {
      ...
      if (_tabController.index == 1 && !_hasLoadedTransactions)
  ...
  Future<void> _loadTransactionData() async {
    ...
    try {
      ...
      if (mounted) {
        if (response['status'] == true && response['data'] is List) {
          setState(() {
            _originalTransactions = response['data'];
  ...
  void _applyTransactionFilters() {
    List<dynamic> result = _filterGenericList(
        _originalTransactions,
        _transactionsSearchController.text,
        (item) => [item['purposeDesc'], item['bankcode']],
        (item) => item['creationDate'],
        activeFilters: _transactionSelectedCategories,
        timePeriod: _transactionSelectedTimePeriod);
  ...
  List<dynamic> _filterGenericList(
    ...
    var filtered = originalList.where((item) {
      final searchCorpus = searchFields(item)
          .where((s) => s != null)
          .map((s) => s!.toLowerCase())
          .join(' ');
      final categorySource = searchFields(item).first?.toLowerCase() ?? '';
      final matchesQuery = q.isEmpty || searchCorpus.contains(q);
      final matchesCategory = activeCategories.isEmpty ||
          activeCategories.any((cat) => categorySource.contains(cat));
      ...
      filtered = filtered.where((item) {
        ...
        switch (timePeriod) {
          ...
          case 'Current Financial Year':
            ...
            return itemDate.isAfter(DateTime(year, 3, 31)) &&
                itemDate.isBefore(DateTime(year + 1, 4, 1));
  ...
  void _showFilterSheet() {
    ...
    final initialCategories = isVoucherTab
        ? _voucherSelectedCategories
        : _transactionSelectedCategories;
    final initialTimePeriod = isVoucherTab
        ? _voucherSelectedTimePeriod
        : _transactionSelectedTimePeriod;
        ...
        builder: (context) => _FilterBottomSheet(
            ...
            onApplyFilter: (newCategories, newTimePeriod) {
              if (isVoucherTab) {
                ...
                setState(() {
                  _transactionSelectedCategories = newCategories;
                  _transactionSelectedTimePeriod = newTimePeriod;
                });
                _applyTransactionFilters();
              }
            }));
      ...
      body: Padding(
        ...
        child: Column(children: [
          TabBar(
              ...
              tabs: const [Tab(text: 'Vouchers'), Tab(text: 'Transactions')]),
          const SizedBox(height: 16),
          Expanded(
              child: TabBarView(
                  controller: _tabController,
                  children: [_buildVouchersTab(), _buildTransactionsTab()])),
        ]),
      ),
    );
  }
        ...
        child: SingleChildScrollView(
            child: Padding(
                ...
                child: Column(
                    ...
                    children: [
                      Image.asset('assets/no_vouchers.webp',
                          width: 200,
                          height: 200,
                      ...
                      if (showButton) ...[
                        ...
                        ElevatedButton(
                            ...
                            style: ElevatedButton.styleFrom(
                                ...
                                shape: RoundedRectangleBorder(
                                    borderRadius: BorderRadius.circular(12))),
                            child: const Text('Issue Vouchers',
        ...
        Expanded(
          child: Column(children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text('${_filteredVouchers.length} AVAILABLE',
                  style: const TextStyle(
                      color: Colors.green,
                      fontWeight: FontWeight.bold,
                      fontSize: 12)),
            ...
            if (_filteredVouchers.isEmpty)
              Expanded(
                  child: _originalVouchers.isEmpty
                      ? _buildEmptyState(
                          message: "No Voucher issued yet!", showButton: true)
                      : _buildEmptyState(
                          message: "No vouchers match your filters."))
            else
              Expanded(
                  child: ListView.builder(
                      padding: EdgeInsets.zero,
                      itemCount: _filteredVouchers.length,
                      itemBuilder: (context, index) => _buildVoucherCard(
                          voucherData: _filteredVouchers[index]))),
          ]),
        )
    ]);
  }
  ...
  Widget _buildTransactionsTab() {
    ...
    return Column(children: [
      Row(children: [
        Expanded(
          child: TextField(
              controller: _transactionsSearchController,
              decoration: InputDecoration(
      ...
      //  else if (_transactionsError != null) Expanded(child: Center(child: Text('Error: $_transactionsError')))
        ...
        Expanded(
            child: _originalTransactions.isEmpty
                ? _buildEmptyState(message: 'You have no transactions yet.')
                : _buildEmptyState(
        ...
        Expanded(
            child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final double totalAmount = transactionsInMonth.fold(0.0,
                      (sum, item) => sum + (item['amount'] as num? ?? 0.0));
                  return Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Container(
  ...
  Widget _buildVoucherCard({required Map<String, dynamic> voucherData}) {
    ...
    String redemptionType =
        (voucherData['redemtionType'] as String?)?.capitalize() ?? 'N/A';
          ...
          children: [
            Row(
              children: [
                ...
                Image.memory(
                  base64ToBytes(voucherData['mccMainIcon']),
                  width: 40,
                  height: 40,
                  fit: BoxFit.contain,
                ),
  ...
  IconData _getIconForPurpose(String purpose) {
    ...
    if (p.contains('meal') || p.contains('food'))
    ...
    return InkWell(
      onTap: () {
        Navigator.push(
            ...
            MaterialPageRoute(
                builder: (context) =>
                    TransactionDetailScreen(transactionData: transactionData)));
      },
      child: Container(
          ...
          children: [
            ...
            Expanded(
              child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(transactionData['purposeDesc'] ?? 'N/A',
                        style: const TextStyle(
                            fontSize: 16,
                            fontWeight: FontWeight.w600,
                            color: Colors.black87)),
                    const SizedBox(height: 4),
                    Text(transactionData['bankcode'] ?? 'Unknown',
                        style: TextStyle(
                    ...
                    Row(children: [
                      ...
                      if (isBillAttached)
                        Chip(
                            label: const Text('Bill Attached'),
